...
class TestDistributedTelepathicObjectConstructor:
    ...
    def test_constructor_accepts_peer_and_object_id(self):
        ...
        distributed_obj = DistributedTelepathicObject(peer=mock_peer, object_id=object_id)
...
class TestCRDTSyncMessageTypes:
    ...
    def test_message_serialization(self):
        ...
        assert deserialized_message.content["object_id"] == original_message.content["object_id"]
        assert deserialized_message.content["operation_data"] == original_message.content["operation_data"]
...
class TestAutomaticHandlerRegistration:
    ...
    def test_setup_sync_handlers_called_during_init(self):
        ...
        with patch.object(DistributedTelepathicObject, '_setup_sync_handlers') as mock_setup:
            distributed_obj = DistributedTelepathicObject(peer=mock_peer, object_id="test")
...
class TestStateRequestResponseCycle:
    ...
    async def mock_distributed_object(self):
        ...
        distributed_obj = DistributedTelepathicObject(peer=mock_peer, object_id="test_obj")
    ...
    async def test_handle_crdt_state_request(self, mock_distributed_object):
        ...
        # Handle the request
        await mock_distributed_object._handle_crdt_state_request(sender_id, request_message)
    ...
    async def test_handle_crdt_state_response(self, mock_distributed_object):
        ...
        response_message = Message("crdt_state_response", {
            "object_id": "test_obj",
        ...
        # Handle the response
        await mock_distributed_object._handle_crdt_state_response(sender_id, response_message)
    ...
    @pytest.mark.asyncio
    async def test_ignore_state_request_for_different_object(self, mock_distributed_object):
        """Test that state requests for different object_id are ignored."""
        # Use our mock Message class instead of importing the real one
        
        # Create state request for different object
        request_message = Message("crdt_state_request", {"object_id": "different_object"})
        sender_id = "requesting_peer"
    ...
    @pytest.mark.asyncio
    async def test_ignore_state_response_for_different_object(self, mock_distributed_object):
        """Test that state responses for different object_id are ignored."""
        # Use our mock Message class instead of importing the real one
        
        response_message = Message("crdt_state_response", {
            "object_id": "different_object",
            "state_data": b"some_data"
        })
        sender_id = "responding_peer"
...
class TestCRDTUpdateHandling:
    ...
    async def mock_distributed_object(self):
        ...
        mock_peer = MagicMock(spec=NetworkPeer)
        distributed_obj = DistributedTelepathicObject(peer=mock_peer, object_id="shared_doc")
    ...
    async def test_handle_crdt_operation_applies_update(self, mock_distributed_object):
        ...
        operation_message = Message("crdt_operation", {
        ...
        await mock_distributed_object._handle_crdt_operation(sender_id, operation_message)
...
class TestOperationBroadcastingOnSetField:
    ...
    def mock_distributed_object(self):
        ...
        mock_peer = MagicMock(spec=NetworkPeer)
        mock_peer.broadcast = AsyncMock(return_value=2)  # Mock 2 peers receiving the broadcast
        
        return DistributedTelepathicObject(peer=mock_peer, object_id="collaborative_doc")
    ...
    async def test_set_field_broadcasts_operation(self, mock_distributed_object):
        ...
        await mock_distributed_object.set_field_async("title", "New Title", "Updated document title")
    ...
    async def test_set_field_calls_parent_method_first(self, mock_distributed_object):
        ...
        with patch.object(TelepathicObject, 'set_field') as mock_parent_set_field:
            await mock_distributed_object.set_field_async("author", "Bob", "Set document author")
            
            # Parent method should be called with same arguments
            mock_parent_set_field.assert_called_once_with("author", "Bob", "Set document author")
    ...
    async def test_operation_includes_recent_changes(self, mock_distributed_object):
        ...
        await mock_distributed_object.set_field_async("title", "Final Title")  # Update title again
...
class TestAutoSyncOnPeerConnect:
    ...
    async def mock_distributed_object(self):
        ...
        distributed_obj = DistributedTelepathicObject(peer=mock_peer, object_id="auto_sync_doc")
    ...
    async def test_peer_connect_triggers_sync_request(self, mock_distributed_object):
        ...
        await mock_distributed_object._handle_peer_status_change(new_peer_id, "connected")
    ...
    async def test_peer_disconnect_no_action(self, mock_distributed_object):
        ...
        await mock_distributed_object._handle_peer_status_change(disconnected_peer_id, "disconnected")
    ...
    async def test_multiple_peer_connections(self, mock_distributed_object):
        ...
        called_peer_ids = {
            call[0][0] for call in mock_distributed_object.peer.send_message.call_args_list
        }
    ...
    async def test_bidirectional_sync(self, mock_distributed_object):
        ...
        response_message = Message("crdt_state_response", {
            "object_id": "auto_sync_doc",
            "state_data": state_data
        })
        
        # Handle their response
        await mock_distributed_object._handle_crdt_state_response(peer_id, response_message)
...
class TestEndToEndIntegration:
    ...
    async def test_three_peer_mesh_sync(self):
        ...
        # Create three peers
        peers = [
            NetworkPeer(handle=f"peer{i}", port=0)
            for i in range(3)
        ]
