...
class TelepathicObject:
    ...
    def set_field(self, path, value):
        ...
        """
        # We need to use dpath, but dpath likes working on dict/list,
        # so we wrap & unwrap on the fly.
        backing = unwrap(self._data)  # plain structure
        dpath.util.new(backing, path, value)  # set the value at path
        # Now, re-wrap the full structure back as CRDTs
    ...
    def load(cls, path):
        ...
        print(f"Data type: {type(obj._data) if hasattr(obj, '_data') else 'No _data attribute'}")
